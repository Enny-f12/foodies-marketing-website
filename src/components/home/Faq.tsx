...
function AccordionItem({ faq, index }: { faq: FAQ; index: number }) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.32, delay: index * 0.04, ease: [0.22, 0.68, 0, 1.1] }}
      ...
      >
        ...
        >
          {open
            ...
            : <Plus size={12} strokeWidth={2.5} color="var(--color-primary)" />
          }
        </motion.div>
      </button>
    ...
    <section className="w-full" style={{ background: "var(--color-bg)" }}>
      ...
      >
        ...
        {/* ════════════════════════════════════════
            ...
            (mirrors the menu category row pattern)
            ════════════════════════════════════════ */}
        <motion.div
          className="faq-mobile-tabs"
          initial={{ opacity: 0, y: 12 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.4, delay: 0.1 }}
        >
          <div className="faq-mobile-scroll no-scrollbar">
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {/* Shortened mobile label */}
                    {cat.label === "Orders & Delivery"  ? "Orders"
                    : cat.label === "Menu & Dietary"    ? "Menu"
                    : cat.label === "App & Technology"  ? "App"
                    : cat.label === "Locations & Hours" ? "Locations"
                    : cat.label === "Catering & Events" ? "Catering"
                    : cat.label === "Loyalty & Rewards" ? "Loyalty"
                    : cat.label}
                  </span>
                </button>
              );
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                style={{
                  ...
                  borderBottom: "2px solid var(--color-text-disabled)",
                }}
              >
                <div
                  style={{
                ...
                >
                  <ActiveIcon size={20} strokeWidth={1.8} color="var(--color-primary)" />
                </div>
                <div>
                  <h3
                    style={{
      ...
      <style>{`
        ...
        .faq-layout {
          display: grid;
          grid-template-columns: clamp(175px, 22%, 230px) 1fr;
          gap: clamp(1.5rem, 4vw, 3rem);
          align-items: start;
        }
        .faq-sidebar { display: block; }

        /* ── Mobile: stack layout, show pill nav ── */
        @media (max-width: 700px) {
          .faq-mobile-tabs { display: block; }
